...
def compute_sp(spike_raster, trials_df, start_idx, end_idx):
    ...
    for i in range(n_neurons):
        fr_0 = np.sum(spike_raster[i, mask_0, start_idx:end_idx], axis=1) / (end_idx - start_idx) # fr_0.shape = (n_trials,)
        fr_100 = np.sum(spike_raster[i, mask_100, start_idx:end_idx], axis=1) / (end_idx - start_idx) # fr_100.shape = (n_trials,)
