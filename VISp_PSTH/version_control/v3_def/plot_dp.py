...
def plot_dp(dp_values_dict, mean_fr_values,
    ...
    """
    DP vs Mean Firing Rate 그래프 생성. (contrast 구별 없음)
    """
    brain_acronym, cluster_type, end_time = plot_info


    fig, ax = plt.subplots(figsize=(6, 5))
    ax.scatter(dp_values_dict, mean_fr_values, c='blue', edgecolors=None, alpha=0.3, s=10)
...
def plot_dp_transformed(dp_values, mean_fr_values,
                        plot_info, save_path=None, save_title=None):
    brain_acronym, cluster_type, end_time = plot_info
    ...
    # DP Scatter Plot (원본)
    axs[0].scatter(dp_values, mean_fr_values, c='blue', edgecolors=None, alpha=0.3, s=10)
    # DP 오차 막대 추가
    bin_centers, means, y_errs, x_errs = compute_binned_stats(dp_values, mean_fr_values)
    ...
    # DP Scatter Plot (변환)
    dp_transformed = 2 * np.abs(dp_values - 0.5)
    axs[1].scatter(dp_transformed, mean_fr_values, c='blue', edgecolors=None, alpha=0.3, s=10)
    # DP 오차 막대 추가
...
def plot_dp_contrast(dp_values_dict, mean_fr_values, 
            plot_info, save_path=None, save_title=None):
    """
    ...
    if save_title is not None:
        ...
        save_file(fig, save_path, f"DP_contrast_SO+{end_time}s_{cluster_type}")



def plot_dp_contrast_transformed(dp_values_dict, mean_fr_values, 
                        plot_info, save_path=None, save_title=None):
    brain_acronym, cluster_type, end_time, contrast_levels, colors = plot_info
    fig, axs = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle(f"[{brain_acronym}] DP SO+{end_time}s (AUC, hit vs miss) - {cluster_type.capitalize()} clusters")
    ...
    if save_title is not None:
        ...
        save_file(fig, save_path, f"DP_contrast_transformed_SO+{end_time}s_{cluster_type}")
